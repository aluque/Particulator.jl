...
using GLMakie
using Particulator
import Particulator: onoutput, ParticleType

struct MakieCallback{T} <: AbstractCallback
    species::Vector{Symbol}
    pos::Vector{NTuple{2, Observable{Vector{T}}}}
end

function MakieCallback(T::Type, species::Vector{Symbol})
    pos = [(Observable(T[]), Observable(T[])) for _ in species]        
    return MakieCallback(species, pos)
end

MakieCallback(species::Vector{Symbol}) = MakieCallback(Float64, species)

function plot!(ax, c::MakieCallback; kw...)
    colors = Dict(
        :electron => :seagreen4,
        :positron => :darkred,
        :photon => :magenta3)
    ...
    for (i, s) in enumerate(c.species)
        (x, y) = c.pos[i]
        scatter!(ax, x, y, color = colors[s])
    end
end


function onoutput(c::MakieCallback, mpopl, t, i=nothing)
    for (i, s) in enumerate(c.species)
        popl = get(mpopl, ParticleType{s})
        (x, y) = c.pos[i]
        x[] = [p.x[1] for p in eachactive(popl)]
        y[] = [p.x[3] for p in eachactive(popl)]        
    end    
end
...
function main(;n_init_particles=1,
    ...
    plotting = MakieCallback([:electron, :photon, :positron])
    plot!(ax, plotting)
    xlims!(ax, -100, 100)
    ylims!(ax, z1 - 5, z2 + 50)
    ax.xlabel = "x (m)"
    ...
    callback = CombinedCallback(callback, (particle_count, thinning, plotting))
